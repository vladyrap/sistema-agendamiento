...
VERIFY_SCHEDULE="${BACKUP_VERIFY_SCHEDULE:-30 3 * * *}"

# Exportar variables de entorno al cron (cron no las hereda en busybox).
mkdir -p /etc/crontabs
{
    env | grep -E "^(POSTGRES_|BACKUP_|B2_|LOCAL_|TWILIO_)" | while IFS= read -r line; do
        printf 'export %s\n' "$line"
    done
...
${VERIFY_SCHEDULE} . /etc/profile.d/backup-env.sh; /usr/local/bin/verify_backup.sh >> /var/log/verify.log 2>&1
EOF

echo "[entrypoint] Backup container listo"
echo "[entrypoint]   Backup schedule:  ${SCHEDULE}"
echo "[entrypoint]   Verify schedule:  ${VERIFY_SCHEDULE}"
echo "[entrypoint]   B2 enabled: $([ -n "${B2_KEY_ID:-}" ] && echo yes || echo no)"
echo "[entrypoint]   WhatsApp notify: $([ -n "${BACKUP_VERIFY_NOTIFY_TO:-}" ] && echo yes || echo no)"
...
echo "[entrypoint] Verify manual:  docker compose exec backup /usr/local/bin/verify_backup.sh"

touch /var/log/backup.log /var/log/verify.log
crond -f -l 8 &
CRON_PID=$!
tail -F /var/log/backup.log /var/log/verify.log &
wait $CRON_PID
